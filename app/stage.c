...
{
    drv_ltdc_fill_layer(0, 0);
    drv_ltdc_fill_layer(1, 0);
    drv_ltdc_deactivate_layer(0);
    drv_ltdc_deactivate_layer(1);

#ifdef SCREEN_X2
    drv_ltdc_set_layer(1,
    ...
    drv_ltdc_enable_clut(0);
    drv_ltdc_enable_clut(1);
...
static void prepare_menu()
{
    ...
    drv_ltdc_set_layer(1, /* This will be for selection */
...
{
    g_drawing_area_p = (uint8_t *)drv_ltdc_get_layer(layer);
}
...
void stage_draw_selection(stage_t stage)
{
    switch(stage)
    {
        case STAGE_EMULATION:
            ...
            break;
        case STAGE_MENU:
            drv_ltdc_move_layer(1, g_icons_a[g_current_icon].x1, g_icons_a[g_current_icon].y1);
            break;
        case STAGE_FILE_SELECT:
            drv_ltdc_move_layer(1, (g_current_file/FILES_IN_COLUMN) * FILENAME_LENGTH_PIXELS, (g_current_file % FILES_IN_COLUMN) * FILENAME_HIGHT);
            break;
        case STAGE_COLOR_SCREEN:
            ;
            break;
