        ...
        switch(g_current_state)
        {
            ...
            case SM_STATE_FILE_SELECT:
                ...
                if(g_key_active_start != 0 &&
                   (drv_timer_get_ms() - g_key_active_start) > LONG_PRESS_MS)
                {
                    if((drv_timer_get_ms() - g_key_active_long_press_delay) > LONG_PRESS_DELAY_MS)
                    {
                        stage_input_key(STAGE_FILE_SELECT, g_key_active);
                        g_key_active_long_press_delay = drv_timer_get_ms();
                    }
                }
...
uint8_t sm_get_ltdc_stats_flag()
{
    return g_ltdc_info;
}
